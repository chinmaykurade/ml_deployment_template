...
# Serializers will help with packing and unpacking database objects into JSON objects. In Endpoints and
# MLAlgorithm serializers, we defined all read-only fields. This is because, we will create and modify our
# objects only on the server-side.For MLAlgorithmStatus, fields status, created_by, created_at and
# parent_mlalgorithm are in read and write mode, we will use the to set algorithm status by REST API.
# For MLRequest serializer there is a feedback field that is left in read and write mode - it will be
# needed to provide feedback about predictions to the server.
#
